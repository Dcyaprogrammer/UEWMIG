...
from utils import encode_missions

minari_dataset = minari.load_dataset("minigrid/BabyAI-Pickup/optimal-fullobs-v0")
...
def get_dataset(
    ...
    for episode in minari_dataset.iterate_episodes():
        direction = episode.observations['direction']
        image = episode.observations['image']
        mission = episode.observations['mission']
        actions = episode.actions
        rewards = episode.rewards
        terminations = episode.terminations
        truncations = episode.truncations
        mission_ids = encode_missions(mission)
        print(image.shape)
...
if __name__ == "__main__":
    # wandb.init(project=config.project, config=config)  # 注释掉
    rng = jax.random.PRNGKey(config.seed)
    # env = gym.make(config.env_name)  # 注释掉
    # 用minari的数据集恢复环境
    ...
    for i in tqdm.tqdm(range(1, num_steps + 1), smoothing=0.1, dynamic_ncols=True):
        ...
        if i == 0 or (i + 1) % eval_interval == 0:
            ...
            print(f"Step {i}, Score: {normalized_score:.2f}")
